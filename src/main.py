...
def run(pipeline_args, base_url, endpoint, start_date, token):
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        if endpoint == 'assignments':
            ...
            query = f'SELECT DISTINCT id FROM `{project_id}.raw_canvas.courses`'
            ...
            query = f'SELECT DISTINCT id, course_id FROM `{project_id}.raw_canvas.assignments`'
        elif endpoint == 'terms':
            endpoint = Terms
            ...
            table_id = f'users'
            query = f'SELECT DISTINCT user_id FROM `{project_id}.raw_canvas.enrollments`'
        else:
            raise
